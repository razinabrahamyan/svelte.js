<script lang="ts">
	...
	async function signup() {
		...
		if (resp.status == 200) {
			const token = generateToken(resp.data);
			$credentials = resp;
			goto('/');
		}
	}
	...
	<div class="mx-auto mt-[15%] mb-[5%] w-full  p-4 lg:w-1/2">
		<div class="mb-8 flex flex-row gap-2">
			...
			<h1 class="text-2xl font-bold text-white lg:text-3xl">
				<div class="text-xs text-surface-300">{env.SITENAME}</div>
				<div class="-mt-1">{$LL.LOGIN_SignUp()}</div>
			</h1>
		</div>
		<div class="-mt-2 mb-2 text-right text-xs text-error-500">{$LL.LOGIN_Required()}</div>
		<form>
			<!-- Username field -->
